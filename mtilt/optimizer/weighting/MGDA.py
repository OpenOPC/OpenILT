...
class MGDA(GradScalerBase):
    ...
    def __init__(
        ...
        self.Grad_Norm_Mode = MGDA_GN_Mode
    ...
    def from_config(cls, cfg):
        ...
        return {
            "config": cfg,
            "MGDA_GN_Mode": cfg.ALGORITHM.MULTI_TASK_LEARNING.MGDA.GRAD_NORM,
        }
    ...
    def _gradient_normalizers(self, grads, loss_data, ntype):
        ...
        grads = grads / gn.unsqueeze(1).repeat(1, grads.size()[1]) # min(grads.data.cpu().numpy()[1][torch.randint(10000,(100,1))])
        return grads
    ...
    def backward(self, losses=None, **kwargs):
        ...
        if self.save_grad:
            ...
            self.save_grads(n_order_grads_show, 'mgda', f'old/sample_{kwargs.get("num", None)}',
                            f'{self.save_grad_order+1}_grads_{kwargs.get("iteration", None)}')

        #
        grads = self._gradient_normalizers(grads, loss_data, ntype=self.Grad_Norm_Mode)  # l2, loss, loss+, none
        #
        sol = self._find_min_norm_element(grads)
        if self.save_grad:
            new_grads = sum([sol[i] * grads[i] for i in range(self.task_num)])
            self.save_grads(new_grads, 'mgda', f'new/sample_{kwargs.get("num", None)}',
                            f'new_grads_{kwargs.get("iteration", None)}')

        #
        if not self.cfg.TRAINER.AMP.ENABLED:
